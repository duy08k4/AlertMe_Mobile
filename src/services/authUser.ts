...
export class authUser {
  ...
  public static async reSendVerification(email: string) {
    ...
    try {
      const { status } = await api.post("/auth/email/resend-verification", {
        email,
      });
  ...
  public static async signout() {
    try {
      const { status } = await api.post("/auth/signout", {
  ...
  public static async updatePassword(
    ...
    authUserGuard.updatePasswordGuard(
      currentPassword,
      newPassword,
      confirmPassword,
    );
