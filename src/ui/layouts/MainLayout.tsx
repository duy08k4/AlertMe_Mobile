...
// Staff menu
const StaffMenu = () => {
    const Menu = useRef<{ label: string, path: string, tab: string, icon: React.ReactNode }[]>([
        {
            label: "B.Đồ",
    ...
    return (
        <IonTabs>
            ...
            <IonRouterOutlet className="z-0">
                ...
                {/* Task */}
                {userAuth ?
                    (<Route path={routeConfig.main.user.myReport} children={<ReportPage />} exact />) :
                    (<Route path={routeConfig.main.staff.myTask} children={<TaskPage />} exact />)
                }
                {/* {staffAuth === true && (<Route path={routeConfig.main.staff.myTask} children={<TaskPage />} exact />)} */}
